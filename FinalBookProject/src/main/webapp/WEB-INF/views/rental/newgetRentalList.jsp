...
<nav>
	...
	<div align="center">
		...
		<div>&nbsp;</div>
		<div>&nbsp;</div>
		<h4 style="margin-right:350px;">전체 도서 목록</h4>
		<div>&nbsp;</div>	
		<!-- 검색 시작 -->
		<form action="./getBooksearch.do" method="get">
			<table id="searchList" border="0" cellpadding="0" cellspacing="0" width="900">
				<tr>
						<td align="right">
						<select name="searchCondition"  style="height:35px; width:60px;" id="searchkeyword" onchange="ChangeValue()">
							<c:forEach items="${ conditionMap }" var="option">
								<option value="${ option.value }">${ option.key }
							</c:forEach>							
						</select> 
						<input type="text"  name="searchKeyword" style="height:35px;" id="inputBox"/> 
						 <!-- 자동완성 단어 리스트 -->

						<button type="submit" value="검색" style="height:35px; width:60px; margin-bottom:5px;" class="btn btn-outline-primary me-2"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-search" viewBox="0 0 16 16">
 ...
 <script>
   ...
   function ChangeValue() {
    	var value_str = document.getElementById('searchkeyword');
    	if(value_str.value == 'AUTHOR'){
    		alert('저자 입니다');
    	}else{
    		alert('제목 입니다');
    	}
    	...
    	if(value_str.value =='TITLE'){ //자바는 참조주소 비교되니깐 equals를쓰지만 다른 데는 문자열 비교시에도 ==씁니다
    		var inputSource = ${searchlist}
            $('#inputBox').autocomplete({
                source : inputSource,
                minLength : 0
		...
		<table class="table-striped table-hover" width="900" style="margin-right:310px;">
			<tr align="center">
				<th bgcolor="#32A4FF" width="100">청구번호</th>
				<th bgcolor="#32A4FF" width="200">제목</th>
				<th bgcolor="#32A4FF" width="150">작가</th>
				<th bgcolor="#32A4FF" width="150">장르</th>
			</tr>
       ...
       <% if (isbn == null) { %>
  	   ...
  	   <% } else { %>
  	   	    <c:forEach items="${isbn}" var="isbn" varStatus="status" >
			<input name="isbn" type="hidden" value="${isbn}" />
				<tr align="center" height="50">
					<td><c:out value="${isbn}" /></td>
					<td align="center"> 
						<a href="getBook.do?isbn=${isbn}">${title[status.index]}</a>    
					</td>		
					<td>${author[status.index]}</td>	
					<td>${genre[status.index]}</td>		
				</tr>
			</c:forEach>	
		<%
...
%>	  	
  ...
  <ul class="pagination">
    ...
    <li class="page-item"><a class="page-link" href="getBookList.do${pageMaker.makeQuery(pageMaker.cri.page - 1)}">Previous</a></li>
    
     <c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
    <li class="page-item"><a class="page-link" href="getBookList.do${pageMaker.makeQuery(idx)}">${idx}</a></li>
    </c:forEach>
    
    <li class="page-item"><a class="page-link" href="getBookList.do${pageMaker.makeQuery(pageMaker.cri.page + 1)}">Next</a></li>
