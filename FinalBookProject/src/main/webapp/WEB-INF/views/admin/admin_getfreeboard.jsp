...
<main>
 ...
 <% String id = (String)session.getAttribute("id"); %>
  ...
  <div>&nbsp;</div>
  <form>
  <input name="seqs" type="hidden" value="${boardreply.seqs}" />
  <input name="seq" type="hidden" value="${boardreply.seq}" />
  <input name="id" type="hidden" value="${boardreply.id}" />
  ...
  작성자 : <input name="id" type="text" style="width:300px;height:30px;" value="${boardreply.id}" disabled/>
  <input name="id" type="hidden" style="width:530px;height:50px;" value="${boardreply.id}" /><br>
  비밀번호 : <input name="password" type="password" style="width:100px;height:30px;" value="${boardreply.password}"/>
  </td>
  <td width="100" height="80" align="right" style="white-space: nowrap;">
  작성일 : ${boardreply.regDate}&nbsp;
  </td>
  </tr>
  <tr style="border-top:1px black solid; ">
  <td width="900" height="300" valign="top" align="left">
  <textarea name="contents" cols="100" rows="16" >&nbsp;${boardreply.contents}</textarea>
  </td>
  </tr>
   </table>
    <div>&nbsp;</div>
    <div style="text-align:center;">
      	<button type="submit" formaction="updateadminreply.do?seq=${boardrely.seq}" class="btn btn-outline-danger" >수정</button>
      	<button type="submit" class="btn btn-outline-primary me-2" formaction="/deleteadminreply.do?seq=${boardrely.seq}" onclick="return confirm('정말 삭제하시겠습니까?')">삭제</button>
 			
 	</form>
     	<button type="button" class="btn btn-outline-warning" onclick="location.href='/getAdminfreeBoard.do?seq=${boardreply.seq}';" >목록</button>
     </div>
  <div>&nbsp;</div>															
