...
#return{
      	width:80px;	background-color:#309AFF;
	...
	img{
	  ...
	  .submitTable{
      	width:100px;	background-color:#309AFF;
      ...
      #imgData{
     	 width:300px; border:hidden; padding-right:20px;
     	 padding-top:30px;
      }
...
<nav>
	<div style="float:left;"class="vertical-menu">
 	 <a href="./insertBook.do" >도서 등록</a>
  	 <a href="./getAdminBookList.do" class="active">도서 수정</a>
  	 <a href="./deleteBook.do" >도서 삭제</a> 	
	</div>
</nav>
...
<div class="bookTable">
	...
	<br>
	<form action="./updateBook.do" name="submitTable" enctype="multipart/form-data" method="post" id="bookForm">
		<input type="hidden" value="${book.isbn}" name="isbn">
		<input type="hidden" value="${book.genre}" name="genre">
		<input type="hidden" value="${book.image}" name="image">
		<table border="1" style="width:1000px;">
			<tr>
				<td id="imgData"valign="top" rowspan="6">		
					<div>
						<img src="bookimage/${book.image}" alt="..." style="border:2px solid black;"/>
					</div>		
				</td>
				<td valign="top" class="columns" style="border-top:hidden;border-right:hidden;">
					제목
				</td>
				<td valign="top" class="values" style="border-top:hidden;border-right:hidden; border-left:hidden;">
			...
			<tr style="border-right:hidden;">
				...
				<td valign="top" class="values" style="border-left:hidden;">
					<input type="text" name="author" value="${book.author}" class="input"/>
				</td>
			</tr>
			<tr style="border-right:hidden;">
				<td valign="top" class="columns">
				...
				<td valign="top" class="values" style="border-left:hidden;">
					<input type="text" name="publisher" value="${book.publisher} " class="input"/>
				</td>
			</tr>
			<tr style="border-right:hidden;">
				<td valign="top" class="columns">
				...
				<td valign="top" class="values" style="border-left:hidden;">
					<input type="text" name="bcount" value="${book.bcount}" class="input"/> 
				</td>
			</tr>
			<tr style="border-right:hidden;">
				<td valign="top" class="columns">
				...
				<td valign="top" class="values" style="border-left:hidden;">
					<textarea name="summary" cols="50" rows="11">${book.summary}</textarea>
				</td>
			</tr>
			<tr style="border-right:hidden;">
				<td valign="top" class="columns">
			...
			<tr style="border-right:hidden; border-bottom: hidden;">
				<td id="lowButton" align="center" valign="middle" colspan="3" style="height:120px; border-bottom:hidden;">
					<br>
					<div id="threeButtons">
					<span style="margin-left:200px;"><input class="submitTable" type="submit" value="수정하기" onclick="return validateForm()"></span>&nbsp;&nbsp;&nbsp;
					<span><button class="submitTable"type="button" onclick="location.href='/getAdminBookList.do';">목록</button></span>&nbsp;&nbsp;&nbsp;
					</div>
				</td>	
			</tr>		
		</table>
	...
	<script>
	 function validateForm() {
	        ...
	        var publisherInput = document.getElementsByName('publisher')[0];
	        var publisher = publisherInput.value.trim();
	        var bcountInput = document.getElementsByName('bcount')[0];
	        var bcount = bcountInput.value.trim();
	        var summaryInput = document.getElementsByName('summary')[0];
	        var summary = summaryInput.value.trim();
	        
	        if (title.length === 0) {
	            alert('제목을 입력해주세요.');
	            titleInput.focus(); // set focus on the title field
	            return false;
	          }
	        
	        if (author.length === 0) {
	            alert('지은이를 입력해주세요.');
	            authorInput.focus(); // set focus on the title field
	            return false;
	          }
	        if (publisher.length === 0) {
	            alert('출판사를 입력해주세요.');
	            publisherInput.focus(); // set focus on the title field
	            return false;
	          }
	        if (bcount.length === 0) {
	            alert('책의 권 수를 입력해주세요.');
	            bcountInput.focus(); // set focus on the title field
	            return false;
	          }
	        if (summary.length === 0) {
	            alert('상세정보를 입력해주세요.');
	            summaryInput.focus(); // set focus on the title field
	            return false;
	          }
	        ...
	        if (fileInput.files.length === 0) {
		        ...
		        fileInput.value = "${book.uploadFile}";
		          return true;
		        }

	        if (!allowedExtensions.exec(filePath)) {
	          alert('jpg, jpeg, png, or gif 파일만 업로드 가능합니다.');
	          fileInput.value = '';
	          window.location.href = window.location.href; 
	          return false;
	        }
	 ...
	 document.getElementById("bookForm").addEventListener("submit", function(event) {
		  ...
		  $.ajax({
			    ...
			    success: function(data) {
			      // alert that the form submission is done
			      alert("도서 수정이 완료되었습니다.");

			      // redirect to the desired page
			      window.location.href = "/getAdminBookList.do";
			    },
			    error: function(jqXHR, textStatus, errorThrown) {
			      // handle the error here
			      alert("도서 수정 실패: " + textStatus + ", " + errorThrown);
			      window.location.href = window.location.href;
			    }
			 });
		});
