...
<main>
  ...
  <div class="b-example-divider"></div>
	...
	<div class="container-fluid px-4">
		<h1 class="mt-4" align="center">User Management</h1>
			&nbsp;
		<div class="card mb-4" style="width:1000px; margin:auto;">
			<div class="card-body" style="width:1000px;">
				<form action="updateAdminmember.do">
					<input type="hidden" name="id" value="${member.id}">
					<div class="mb-3 mt-3">
						<label for="bno" class="form-label">아이디</label> <input type="text"
							class="form-control" id="id" value="${member.id}" disabled>
					</div>
					<div class="mb-3">
						<label for="title" class="form-label">이름</label> <input
							type="text" class="form-control" id="name" name="name"
							value="${member.name}">
					</div>
					<div class="mb-3">
						<label for="title" class="form-label">이메일</label> <input
							type="text" class="form-control" id="email" name="email"
							value="${member.email}">
					</div>
					<a href="./getmemberList.do" class="btn btn-outline-secondary">목록으로</a> 
					<input class="btn btn-outline-warning" type="submit" value="정보 수정" onclick="return validationForm()">
					<a href="deleteAdminmember.do?id=${member.id}" class="btn btn-outline-danger"
						onclick="return confirm('정말로 해당 회원을 탈퇴시키겠습니까?')">회원 탈퇴</a>
				</form>
			</div>
		</div>
	</div>
